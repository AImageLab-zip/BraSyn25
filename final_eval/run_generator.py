...
parser.add_argument('--more',type=str,default='',help='Provide a string that will be concatenaded with the id during the saving phase')
...
input_path = Path(f'/work_path/brats3d/pseudo_random/original')

# These paths will contain the 3 original input modals + the reconstructed modal
output_path_gli_complete = Path(f'/work_path/brats3d/pseudo_random/complete_recon_gli_{id}{more}') # nnunet convention
output_path_met_complete = Path(f'/work_path/brats3d/pseudo_random/complete_recon_met_{id}{more}') # brats convention

# These paths will only contain the reconstructed modality, in nnunet_convention
output_path_gli = Path(f'/work_path/brats3d/pseudo_random/recon_gli_{id}{more}')
output_path_met = Path(f'/work_path/brats3d/pseudo_random/recon_met_{id}{more}')

# Remember that only checkpoints available in the "final" directory will be usable 
weights_file = Path(f'/work_path/checkpoints/final/{id}.safetensors')
