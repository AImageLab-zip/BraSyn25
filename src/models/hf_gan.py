            ...
            self.downs.append(
                nn.ModuleList(
                    [
                        ...
                        DownSample(dim_out, groups=self.groups) if not is_last else nn.Identity(), # Downsampling at each layer, except the last
                    ]
                )
            ) 
...
class HFGAN(nn.Module):
    def __init__(self, dim, num_inputs, num_outputs, dim_mults, n_layers, skip, blocks, image_size=240, grouped_encoder=False, infuse_view = False,num_heads=16):
        super().__init__()
        ...
        if self.infuse_view:
            self.middle = ModalityViewInfuser(hidden_size=dim*dim_mults[-1],  patch_size=1, n_tokens=n_tokens, n_layers=n_layers, n_heads=num_heads, modality_embed=True)
        else:
            self.middle = ModalityInfuser(hidden_size=dim*dim_mults[-1],  patch_size=1, n_tokens=n_tokens, n_layers=n_layers, n_heads=num_heads, modality_embed=True)
        ...
        for i in range(0,155,self.batch_size):
            ...
            with torch.no_grad():
                recon, _ = self.model(brain_slice.permute(3,0,1,2).contiguous(),
                                            [[x for x in range(4) if x!=(missing_modality) ] for _ in range(self.batch_size)],
...
class HFGAN_3D(nn.Module):
    ...
    def __init__(self,
        ...
        model = HFGAN(dim=64, num_inputs=4, num_outputs=1, dim_mults=(1,2,4,8,10), n_layers=4, skip=True, blocks=False,grouped_encoder=grouped_encoder, infuse_view=infuse_view).cuda()
