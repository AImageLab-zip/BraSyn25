...
def perform_inference_step(output_folder, input_folder_nnunet):
    """
    ...
    results_inference_path= [
        join(output_folder, 'Dataset233_BraTS_Naida', 'nnUNetTrainer__nnUNetPlans__3d_fullres_new'),
        join(output_folder, 'Dataset233_BraTS_Naida', 'nnUNetTrainer_SwinUNETR__nnUNetPlans__3d_fullres_SwinUNETR_new'),
        join(output_folder, 'Dataset233_BraTS_Naida', 'nnUNetTrainerBN_BS5_RBT_DS_BD_PS__nnUNetPlans__3d_fullres_BN_BS5_RBT_DS_BD_PS_new'),
        join(output_folder, 'Dataset236_BraTS_GANs', 'nnUNetTrainer__nnUNetPlans__3d_fullres'),
        join(output_folder, 'Dataset236_BraTS_GANs', 'nnUNetTrainer_SwinUNETR__nnUNetPlans__3d_fullres_SwinUNETR'),
        join(output_folder, 'Dataset236_BraTS_GANs', 'nnUNetTrainerBN_BS5_RBT_DS_BD_PS__nnUNetPlans__3d_fullres_BN_BS5_RBT_DS_BD_PS'),
        join(output_folder, 'Dataset238_BraTS_rGANs', 'nnUNetTrainer__nnUNetPlans__3d_fullres'),
        join(output_folder, 'Dataset238_BraTS_rGANs', 'nnUNetTrainer_SwinUNETR__nnUNetPlans__3d_fullres_SwinUNETR'),
        join(output_folder, 'Dataset238_BraTS_rGANs', 'nnUNetTrainerBN_BS5_RBT_DS_BD_PS__nnUNetPlans__3d_fullres_BN_BS5_RBT_DS_BD_PS')
    ]
    ...
    ensemble_code = 'GB_GL_GS_RB_RL_RS_rGB_rGL_rGS'
    ensemble_prob_path = join(output_folder, 'ensemble', ensemble_code, 'prob')
    maybe_mkdir_p(ensemble_prob_path)

    print(f"First inference: {all_commands[0]}",flush=True)
    print("__")
    ...
    print(f"Second inference: {all_commands[1]}",flush=True)
    print("__")
    ...
    ensemble_predictions(inference_folder=inference_output_folder, ensemble_prob_path=ensemble_prob_path, number_models=9, first_infer=False)
    shutil.rmtree(inference_output_folder)

    print(f"Third inference: {all_commands[2]}",flush=True)
    print("__")
    ...
    print(f"Fourth inference: {all_commands[3]}")
    print("__")
    result = subprocess.run(all_commands[3], shell=True, text=True, capture_output=True)
    print("Command output:")
    print(result.stdout)
    inference_output_folder = all_commands[3].split("-o ")[-1].split(" -f")[0]
    ensemble_predictions(inference_folder=inference_output_folder, ensemble_prob_path=ensemble_prob_path, number_models=9, first_infer=False)
    shutil.rmtree(inference_output_folder)

    print(f"Fifth inference: {all_commands[4]}")
    print("__")
    result = subprocess.run(all_commands[4], shell=True, text=True, capture_output=True)
    print("Command output:")
    print(result.stdout)
    inference_output_folder = all_commands[4].split("-o ")[-1].split(" -f")[0]
    ensemble_predictions(inference_folder=inference_output_folder, ensemble_prob_path=ensemble_prob_path, number_models=9, first_infer=False)
    shutil.rmtree(inference_output_folder)

    print(f"Sixth inference: {all_commands[5]}",flush=True)
    print("__")
    result = subprocess.run(all_commands[5], shell=True, text=True, capture_output=True)
    print("Command output:")
    print(result.stdout)
    inference_output_folder = all_commands[5].split("-o ")[-1].split(" -f")[0]
    ensemble_predictions(inference_folder=inference_output_folder, ensemble_prob_path=ensemble_prob_path, number_models=9, first_infer=False)
    shutil.rmtree(inference_output_folder)

    print(f"Seventh inference: {all_commands[6]}")
    print("__")
    result = subprocess.run(all_commands[6], shell=True, text=True, capture_output=True)
    print("Command output:")
    print(result.stdout)
    inference_output_folder = all_commands[6].split("-o ")[-1].split(" -f")[0]
    ensemble_predictions(inference_folder=inference_output_folder, ensemble_prob_path=ensemble_prob_path, number_models=9, first_infer=False)
    shutil.rmtree(inference_output_folder)

    print(f"Eighth inference: {all_commands[7]}")
    print("__")
    result = subprocess.run(all_commands[7], shell=True, text=True, capture_output=True)
    print("Command output:")
    print(result.stdout)
    inference_output_folder = all_commands[7].split("-o ")[-1].split(" -f")[0]
    ensemble_predictions(inference_folder=inference_output_folder, ensemble_prob_path=ensemble_prob_path, number_models=9, first_infer=False)
    shutil.rmtree(inference_output_folder)

    print(f"Ninth inference: {all_commands[8]}")
    print("__")
    result = subprocess.run(all_commands[8], shell=True, text=True, capture_output=True)
    print("Command output:")
    print(result.stdout)
    inference_output_folder = all_commands[8].split("-o ")[-1].split(" -f")[0]
    ensemble_predictions(inference_folder=inference_output_folder, ensemble_prob_path=ensemble_prob_path, number_models=9, first_infer=False)
    shutil.rmtree(inference_output_folder)

    # Creating labels from prob
    ensemble_label_path = join(output_folder, 'ensemble', ensemble_code, 'raw')
...
def code_translator(code, inference_folder):
    ...
    """
    translator_dict = {
        'GB': 'Dataset236_BraTS_GANs/nnUNetTrainer__nnUNetPlans__3d_fullres',
        'GL': 'Dataset236_BraTS_GANs/nnUNetTrainerBN_BS5_RBT_DS_BD_PS__nnUNetPlans__3d_fullres_BN_BS5_RBT_DS_BD_PS',
        'GS': 'Dataset236_BraTS_GANs/nnUNetTrainer_SwinUNETR__nnUNetPlans__3d_fullres_SwinUNETR',
        'RB': 'Dataset233_BraTS_Naida/nnUNetTrainer__nnUNetPlans__3d_fullres_new',
        'RL': 'Dataset233_BraTS_Naida/nnUNetTrainerBN_BS5_RBT_DS_BD_PS__nnUNetPlans__3d_fullres_BN_BS5_RBT_DS_BD_PS_new', 
        'RS': 'Dataset233_BraTS_Naida/nnUNetTrainer_SwinUNETR__nnUNetPlans__3d_fullres_SwinUNETR_new',
        'rGB': 'Dataset238_BraTS_rGANs/nnUNetTrainer__nnUNetPlans__3d_fullres',
        'rGL': 'Dataset238_BraTS_rGANs/nnUNetTrainerBN_BS5_RBT_DS_BD_PS__nnUNetPlans__3d_fullres_BN_BS5_RBT_DS_BD_PS',
        'rGS': 'Dataset238_BraTS_rGANs/nnUNetTrainer_SwinUNETR__nnUNetPlans__3d_fullres_SwinUNETR'
...
def infer():
    ...
    parser.add_argument('--id',type = str,required=True,help='Provide the base identifier of the checkpoint.\n'+
                        'Ex: if the checkpoint is at checkpoints/final/run_14_always_three.safetensors,\n' +
                        'the id should be: --id run_14_always_three')
    args = parser.parse_args()
    id = args.id

    # Setting the path for the weights
    os.environ['nnUNet_results'] = '/work_path/gli_segm_nnunet_chk'
    os.environ['nnUNet_preprocessed'] = 'NONE'
    os.environ['nnUNet_raw'] = 'NONE'

    input_folder_nnunet = Path(f'/work_path/brats3d/pseudo_random/complete_recon_gli_{id}') # nnunet convention
    output_path = f'/work_path/brats3d/pseudo_random/gli_segm_{id}'

    # Doing the inference thing
    inference_folder =  f"/work_path/tmp/inference_{id}/"
    if os.path.exists(inference_folder):
        shutil.rmtree(inference_folder)
    os.makedirs(inference_folder)
    perform_inference_step(output_folder=inference_folder, input_folder_nnunet=input_folder_nnunet)
